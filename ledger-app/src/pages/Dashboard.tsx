...
import { useMonthData, type Transaction, type FixedCostWithPayment } from '../hooks/useMonthData'
...
import { EditFixedCostModal } from '../components/dashboard/EditFixedCostModal'
...
export function Dashboard() {
  ...
  const [editingFixedCost, setEditingFixedCost] = useState<FixedCostWithPayment | null>(null)
          ...
          <div className="space-y-3">
            {fixedCosts.map((fc) => (
              <FixedCostRow
                key={fc.id}
                item={fc}
                onTogglePaid={toggleFixedCostPaid}
                onChangeAmount={setFixedCostAmount}
                onEdit={setEditingFixedCost}
              />
            ))}
          </div>
        )}
