...
export function Goals() {
  const { goals, addGoal, contributeToGoal } = useData()
    ...
    <div className="space-y-6 pb-6">
      ...
      {goals.length > 0 && (
        <div className="bg-surface-container-low p-4 rounded-2xl">
          ...
          <div className="flex justify-between mt-1.5">
            ...
            <span className="text-xs text-on-surface-variant">{(100 - pct).toFixed(1)}% para a meta</span>
          </div>
        </div>
      )}
      ...
      {goals.length === 0 ? (
        ...
        <div className="space-y-3">
          {goals.map((g) => (
            <GoalCard key={g.id} goal={g} onContribute={contributeToGoal} />
          ))}
        </div>
      )}
